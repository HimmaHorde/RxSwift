...
Swift拥有强大的类型系统，可以用来提高应用程序的正确性和稳定性，使Rx的使用更加直观。
...
_**Note:** 本文档中描述的一些特性 (例如 `Driver`) 只针对 [RxCocoa](https://github.com/ReactiveX/RxSwift/tree/master/RxCocoa) 工程, 有些是通用工程 [RxSwift](https://github.com/ReactiveX/RxSwift) 的一部分 . However, the same principles could easily be implemented in other Rx implementations, if necessary. There is no private API magic needed._

### 实现原理

特征是带有一个只读属性可观察序列的结构体
...
您可以将它们看作一种可观察序列的生成器模式实现。当构建一个特征时，调用' . asobservable() '将把它转换回一个普通的可观察序列。

---

## RxSwift 的特征
	Single 、Completable、Maybe 都是 PrimitiveSequence<Trait, Element> 的别名
### Single

Single 是普通可观察序列的变体，只发出一个元素或一个错误。
...
或者使用 `subscribe(onSuccess:onError:)` 如下: 
```swift
getRepo("ReactiveX/RxSwift")
